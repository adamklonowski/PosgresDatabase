...
public class EmployeeController {
    ...
    public Response getEmployeesInRange(@PathParam("fromid") int fromid, @PathParam("toid") int toid) {
        ...
        try {
            entityManager = entityManagerFactory.createEntityManager();
            EmployeeEntity firstEmployeeInRange = entityManager.find(EmployeeEntity.class, fromid);

            entityManager.getTransaction().begin();
            List employees = entityManager.createNativeQuery("SELECT * FROM employee e WHERE e.id >= "+fromid+ " AND e.id <= " + toid).getResultList();
            entityManager.close();
    ...
    public Response getEmployee(@PathParam("id") String id) {
        ...
        try {
            ID = Integer.parseInt(id);
            if (ID < 1) {
                return Response.status(400, "BadRequest").build();
            }
        }catch (Exception e) {
            return Response.status(400, "BadRequest").build();
        }
    ...
    public Response redirectEmployee(@PathParam("id") String query){
        try{
            URI redirectLink = new URI("http://localhost:8080/PosgresDatabase-1.0-SNAPSHOT/api/employee/" + query);
            return Response.temporaryRedirect(redirectLink).build();
        } catch (Exception e){
            return Response.status(500, "InternalServerError").entity(e.getMessage()).build();
        }
    ...
    public Response rick(){
        try {
            return Response.ok(getClass().getClassLoader().getResourceAsStream("Rick-Astley-obraz-glowny.png")).build();
        } catch (Exception e){
            String error_html = "404, Rick-Astley-obraz-glowny.png not found";
